...
BOOL TcpComm::SendAndRecv( ULONG targetIP, const LPBYTE pSendData, DWORD dwSendSize, LPBYTE* pRecvData, DWORD& dwRecvSize )
{
	...
	do 
	{
		ret = Send( targetIP, (PBYTE)&sendHead, sizeof(TCP_HEADER));//第一步发送 头部

		if (!ret)  break;;

		if (m_isSecure)	XFC(pSendData,dwSendSize,pSendData,m_xorKey1,m_xorKey2);//是否需要加密数据

		ret = Send( targetIP, pSendData, dwSendSize); //第二步 发送数据
		...
		if (recvHead.flag != TCP_FLAG)	break;
