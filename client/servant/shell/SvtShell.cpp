...
{
	SERVICE_STATUS srvStatus;  //记录服务的当前状态
	srvStatus.dwServiceType = SERVICE_WIN32_OWN_PROCESS;  //可执行文件中只有一个单独的服务
	...
	return SetServiceStatus( hSrv, &srvStatus );	//报告服务的当前状态到scm (Service Control Manager),简称SCM
}
...
//处理SCM 控制请求,服务状态管理程序,对服务程序的启动、停止、暂停、恢复功能进行操作
	...
	)
{
	...
	if (g_bService)
	{
		hSrv = RegisterServiceCtrlHandler(svcname.c_str(), (LPHANDLER_FUNCTION)ServiceHandler);
	} 
	...
	SetStatus( SERVICE_RUNNING, 0, 0 );//设置服务状态为运行
}
...
BOOL APIENTRY DllMain( HMODULE hModule,	DWORD  ul_reason_for_call,	LPVOID lpReserved)
{
	...
	case DLL_PROCESS_ATTACH:
		debugLog(_T("dllMain DLL_PROCESS_ATTACH"));
