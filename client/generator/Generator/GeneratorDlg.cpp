...
static BOOL FindAndSet(LPBYTE pBase, DWORD dwSize, DWORD dwFlag, LPBYTE pData, DWORD dwDataSize, 
						BOOL bEncrypt = FALSE, int factor1 = 0, int factor2 = 0)
{
	int count = 0;
	LPBYTE pTest = pBase;
...
BOOL WriteSetup(CONNECT_INFO& config,SERVICE_INFO& service,CString& strError)
{
	...
	strSavePath.Format(_T("%s%s"), szPath, BINGO_PATH); //bingo
	CreateDirectory(strSavePath, NULL);


	strTempServant.Format(_T("%s\\%s"), strSavePath, SERVANT_FILE);		//body.dll
	strNSIFilePath.Format(_T("%s\\%s"), strSavePath, NSI_FILE);			//packet.nsi
	strDataServant.Format(_T("%s\\%s"), strSavePath, SERVANT_DATA_FILE);//data.dat
	strShellPath.Format(_T("%s\\%s"),	strSavePath, SHELL_FILE);		//shell.dll
	strShellBak.Format(_T("%s\\%s"),	strSavePath, SHELL_BAK_FILE);	//shell.bak
	...
	if(!ResourceToFile(strNSIFilePath,MAKEINTRESOURCE(IDR_RC_NSI),L"RC_NSI", szInstallPath,4096,key1,key2))
	{
		strError = _T("导出资源失败!");
		return FALSE;
	}
