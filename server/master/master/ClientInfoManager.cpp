...
{
	TStringVector clientidList;  //保存客户端id 
	CommManager::GetInstanceRef().ListAvailableClient(clientidList); //有效的客户端id
			...
			if (infoIter == m_clientBaseInfoMap.end())
			{
				...
				if (infoIter->second.bValid && infoIter->second.bAdd)
				{
					...
					if (NULL != m_fnCallback) {
						m_fnCallback(WM_ADD_CLIENT, (LPVOID)&info, m_lpParameter);
					}
